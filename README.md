...
| `audio.js` | Web Audio 룸톤·글리치·다층 방송 합성 보이스·엔딩음·상태별 상호작용음 (assets 파일 우선) |
| `game.js` | 7블록 진행, 해금 체인, 중간 저장/불러오기, 목표 바, 단서 노트, 형상 렌더러, NG+, 엔딩 분기 |
| `test/chain-test.js` | 헤드리스 전체 진행 검증 (`npm i jsdom@22` 후 `node test/chain-test.js`) |

## 1회차 7블록 진행 (추가안 4)

1. **방송 관찰** — 이상 현상 6종 중 2개를 클릭해 발견 → filesystem 해금
2. **filesystem 탐색** — 잠긴 폴더마다 해금 조건이 힌트로 표시됨
3. **채팅 파싱 복원** — `parser_recovery.exe`, 핵심 3개만 먼저 복원. 나머지 7개는 재실행 시 선택 콘텐츠
4. **routine_queue 해제** — `rest_refusal.log`에 쌓인 순서대로 (잠→다음 방송→나중에) 무효화. 틀리면 새 작업이 생성되고 실패 카운트가 쌓임
5. **반죽이 기록 대조** — 기록 2개의 original/displayed 비교 → 왜곡 규칙 추론 퀴즈
6. **recordings/ 클립 검토** — 다시보기 2개를 핵심 검토하면 진행. 세 번째는 선택 기록
...
라이브 조사 콘솔과 모든 목표 바에는 챕터·퍼센트 진행도가 표시되며,
20% 단위로 진행 완료 피드백이 출력된다.

라이브 상단 또는 Windows 작업표시줄의 **저장/불러오기**로 한 개의 중간 저장을
유지할 수 있다. `Ctrl/Cmd+S`, `Ctrl/Cmd+L`도 지원하며, 저장이 있으면 첫 화면에
진행률이 표시된 **이어하기**가 나타난다. 엔딩에 도달한 회차의 중간 저장은 자동으로 닫힌다.
...
2회차부터 `returning_viewer.log`, 이전 회차 흔적과 오염 채팅이 추가된다.
첫 화면의 **다회차 빠른 진행** 또는 라이브의 **다회차 검증 구간 스킵**을 사용하면
이미 해결한 관찰·신호 복구·루틴 종료·기록/클립 대조를 한 번에 완료한다.
복원 소스와 엔딩 선택은 건너뛰지 않으므로 회차별 분기는 그대로 유지된다.
routine_queue 화면에서도 다회차 권한으로 해당 루틴만 따로 종료 처리할 수 있다.
...
위장 방송과 다시보기에는 저역 진행자층·불안정한 중역 포먼트·좌우 숨소리를
자막 단위로 합성해, 짧은 음성 조각 사이도 하나의 비정상적인 방송처럼 이어지게 한다.
